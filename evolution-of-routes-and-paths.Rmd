...
```{r}
run_sim <- function(net, alpha = 1.05, beta = 0.2, k = 1, epsilon = 0.5, G = 0.1, a = 0.2, b = .0005){
  tmp <- net %>%
    get_distance  %E>%
    # interact
    mutate(utility = .N()$attractiveness[to] ^ alpha * exp(-beta * effective_distance))  %N>%
...
```{r message = FALSE}
sim_length <- 100 # number of time steps
...
sim[[sim_length - 1]] %E>%
 ...
 # nystuen_dacey %>%
ggraph(x = x, y = y) + 
  geom_edge_fan(aes(color = log(total_traffic)), width = 1.2) +
...
param_sweep <- expand.grid(alpha = seq(1, 1.5, by = 0.05),
  ...
  mutate(sim = future_map2(alpha, beta, function(x, y) reduce(1:sim_length, ~run_sim(., alpha = x, beta = y, a = 1), .init = paths), .progress = TRUE))
```
