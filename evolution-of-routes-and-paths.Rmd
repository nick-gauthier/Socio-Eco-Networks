...
These networks act as social infrastructure, channeling the flow of energy between spatially structured populations in much the same way as food webs channel energy in ecosystems \parencite{Crabtree2017ReconstructingStates;Crabtree2015}.
...
```{r}
#paths <- create_lattice(c(4, 4)) %N>%
#  mutate(population = pop_start,
#         attractiveness = 1,
#         x = rep(c(20, 40, 60, 80), 4),
#         y = rep(c(20, 40, 60, 80), each = 4))

paths <- create_complete(16) %N>%
  mutate(population = pop_start,
         attractiveness = 1,
         x = rep(c(20, 40, 60, 80), 4),
...
ggraph(routes, x = x, y = y) + 
  geom_edge_link(alpha = .5, aes(color = distance)) +
  geom_node_point(aes(size = centrality_degree(weights = -1*distance))) +
  coord_equal() +
  theme_void()
```
...
settlements %E>%
  group_by(from) %>% 
  top_n(-6, distance) %>% # retain the top 3 shortest edges
ggraph(x = x, y = y) +
  geom_node_point(aes(size = population)) +
  geom_edge_link(alpha = .5) +
  ...
  scale_size_area() +
  coord_equal() +
  ggtitle('Initial population distribution', 'k = 4 nearest neighbor network') +
