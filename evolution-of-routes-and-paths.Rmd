...
test <- routes %E>%
  as_tibble %>%
  mutate(test = future_map2(from, to, ~as.numeric(shortest_paths(paths, .x, .y, output = 'epath')$epath[[1]]), .progress = TRUE))
...
test2 <- sim %>%
 ...
 # as_tibble %>%
  select(time, from, to, flow) %>%
  left_join(test, .) %>%
  select(from, test, time, flow) %>%
  group_by(time, from) %>%
  nest %>%
  mutate(data = future_map(data, ~unnest(., test))) %>%
  select(data) %>%
  unnest(cols = data) %>%
  group_by(test) %>%
