...
patches-own [ wheat-prop productivity ]  ;; each patch has its own randomly varying productivity value
...
to plant
  set [ wheat-prop ] of patch-here ((118 - 25 * drought-risk) / (278 - 185 * drought-risk))
end


to rain
  ;; droughts occur with probability drought-risk
  ;; if homoneous-rain? is true, pull a single random number for drought occurrence and
  ;; update all patches simultaneously.
  ;; if homogenous-rain? is false, generate random numbers and update all patches separately
  ifelse homogenous-rain? = True [
    ifelse random-float 1 < drought-risk [
      ask patches [
        set productivity 0
        set pcolor red
      ]
