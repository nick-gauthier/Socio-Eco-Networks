...
expand_grid(nu = c(.02, .06, .12),
            welfare = (1:150) / 100) %>%
  mutate(production = .05 + (.5 - .05) * plogis(welfare, scale = nu, location = 1)) %>%
...
$$
    E_{ij} = \frac{O_i W_j^\alpha e^{-c_{ij}/\beta}}{\sum_kW_k^\alpha e^{-c_{ik}/\beta}},
$$
...
```{r}
migrate <- function(net, alpha1, alpha2, beta, nu = .05, m_min = 0.05, m_max = 0.5){
  net %E>%
    ...
    mutate(migrant_balance = centrality_degree(weights = migrant_utility, mode = 'out', loops = TRUE),
           migrant_production = population * (m_min + (m_max - m_min) * plogis(population / harvest, scale = nu, location = 1))) %E>% 
...
The entire system is composed of complicated nonlinear equations, as the growth and decline of settements can potentialy depend on al other settlements. The complexity of this system increases geometrically as the size of the social-ecological network increases,
precluding exact analytic solutions. We must instead leverage computational simulations to capture these
complex interactions numerically. Refer to @Anderies2011 for analytical exploration of a similar model. Now we use simulation to better understand the behavior of this model in different enviromental contexts.

```{r}
run_sim <- function(net, alpha1 = 1.15, alpha2 = 1, beta1 = 5, beta2 = 10, nu, m_max){
  new_net <- net %>%
    trade(alpha = alpha1, beta = beta1) %>%
    migrate(alpha1 = alpha1, alpha2 = alpha2, beta = beta2, nu = nu, m_max = m_max) %>%
    grow %>%
...
We only assume that all $\alpha >= 1$, that is there are positive returns to scale in attractiveness, and 
...
param_sweep <- readRDS('param_sweep') %>%
  mutate(nodes = map(sim, as_tibble, 'nodes'),
         edges = map(sim, as_tibble, 'edges'),
        nd_trade = map(sim, nystuen_dacey),
         nd_migrants = map(sim, nystuen_dacey, mode = 'migrants'))
```


## Distance deterrence and spatial patterning

How does the cost of moving food and people over distances, as encoded in the $\beta$ parameters, influence settlement patterns at equilibrium. The $\beta$ parameters are the primary way space is introduced into the model. Critically, the costs -- be they in time, money, or energy -- of moving food and other resources from hinterlands to settlements will be different from the costs of moving people between settlements. The balance of these cost factors can introduce complexity via non linear interactions between the two. A settlement's trading catchment can be different from their migration catchment, so nontrivial spatial patterns can result from these interactions. Here I focus on the parameter space where $\beta_1 \leq \beta_2$, because moving bulk food supplies over the landscape should be more difficult than moving people. 
...
a <- param_sweep %>%
  ...
  filter(alpha1 == 1.15, alpha2 == 1, m_max == 0.05, beta2 == 20) %>%
...
## Settlement attractiveness and returns to scale
...
param_sweep %>%
  ...
  coord_equal() +
  theme_void() +
  facet_grid(rows = vars(nu), cols = vars(m_max))
```
```{r}
knitr::knit_exit()
```
...
dat %>%
  filter(m_max == 0.05) %>%
mgcv::gam(population ~ s(beta1, k = 4) + s(beta2, k = 4) + s(alpha1, k = 4) + s(alpha2, k = 4),
          data = .) %>% plot
```
...
dat %>%
  filter(m_max == 0.5) %>%
mgcv::gam(population ~ s(beta1, k = 4) + s(beta2, k = 4) + s(alpha1, k = 4) + s(alpha2, k = 4) + s(nu, k = 3),
          data = .) %>% plot
```
...
dat %>%
  filter(m_max == 0.05) %>%
mgcv::gam(population ~ s(beta1, beta2, k = 8) + s(alpha1, alpha2, k = 8),
          data = .) %>% plot(scheme = 1)
```
...
dat %>%
  filter(m_max == 0.05) %>%
mgcv::gam(entropy ~ s(beta1, k = 4) + s(beta2, k = 4) + s(alpha1, k = 4) + s(alpha2, k = 4),
          data = .) %>% plot
```
...
dat %>%
  filter(m_max == 0.5) %>%
mgcv::gam(entropy ~ s(beta1, k = 4) + s(beta2, k = 4) + s(alpha1, k = 4) + s(alpha2, k = 4) + s(nu, k = 3),
          data = .) %>% plot
```
...
dat %>%
  filter(m_max == 0.5) %>%
mgcv::gam(count ~ s(beta1, k = 4) + s(beta2, k = 4) + s(alpha1, k = 4) + s(alpha2, k = 4) + s(nu, k = 3),family = poisson, data = .) %>% plot
```
...
dat %>%
  filter(m_max == 0.05) %>%
mgcv::gam(count ~ s(beta1, k = 4) + s(beta2, k = 4) + s(alpha1, k = 4) + s(alpha2, k = 4),family = poisson, data = .) %>% plot
```
...
param_sweep %>%
  filter(beta1 == 10, beta2 == 15, m_max == .05)  %>%
    mutate(nd = map(nd_trade, as_tibble, 'edges')) %>%
    select(beta1:m_max, nd) %>%
    unnest(nd) %>%
  select(alpha1, alpha2, from, to) %>%
  tbl_graph(nodes = as_tibble(paths, 'nodes'), edges = .) %>%
  ggraph(x = x, y = y) +
  geom_edge_link(alpha = .5) +
  facet_edges(alpha1 ~ alpha2) +
  coord_equal() +
  theme_void()
```
...
```{r}
param_sweep %>%
  filter(alpha1 == 1.15, alpha2 == 1.15, m_max == .05)  %>%
    mutate(flows = map(sim, as_tibble, 'edges')) %>%
    select(beta1:m_max, flows) %>%
    unnest(flows) %>% 
...
param_sweep %>%
  ...
  mutate(edges = map(edges, ~filter(., trade_flow > 1 | migrant_flow > 1)))
  unnest(col = c(edges))
```
...
WHAT ABOUT THE TIME TILL EQUILIBRIUM
...
sim4 <- settlements %>%
 ...
 # mutate(EOF = spatial_patterns$constant) %>%
  accumulate(1:500, ~run_sim(.x), .init = .)

#temporal_patterns$high
```
...
First we allow an external climate forcing function to influence resource-system dynamics. We assume a water-limited environment, so that the carrying capacity of the resource represents the limitation of soil moisture on resource abundance. We thus replace the static parameter X with some unknown function of climatic water balance.
...
RFoptions(seed=100)
spatial_patterns <- settlements %N>%
...
```
Rainfall varies normally in time, but it is important that crop yields do not vary linearly with rainfall. Yield should be a declining function of yield, such that given normal variability the risk of crop failure is higher than if there hadnt been any.
Simple power function of rainfall is crop production function

Yields are modeled relative to their baseline value in a normal rainfall year. This is the same production as in @Janssen2011, although scaled to units of SPEI rather than PDSI.

$$
  X_i = 1.5 \hat{X_i}\left(1 - \exp\left(\frac{\ln(1/3)}{4} (\mathit{SPEI_i} - 4)\right) \right)
$$
where SPEI is the standardized precipitation evapotranspiration index. I model variation in SPEI as a simple AR(1) process

$$
     \mathit{SPEI}_{t+1} = \phi \mathit{SPEI}_t + \epsilon
$$
Where $\epsilon$ is drawn from a standard normal distribution and $\phi$ is controls the strength of autocorrelation between time steps. 

This is the global SPEI value, it gets projected onto a particular grid cell $i$ by multiplyiing by a spatial anomaly $a$ as $\mathit{SPEI_i} = a \mathit{SPEI}$
```{r}
yield <- function(x){
  pmax(1.5 * (1 - exp(log(1/3) / 4 * (x + 4))), 0)
}

weather <- function(SPEI, phi = 0){
 phi * SPEI + rnorm(1)
}

produce_food <- function(net, SPEI){
  net %N>%
    mutate(yield = yield(EOF * SPEI))
}

t1 <- accumulate(1:100, ~weather(.x, phi = .6), .init = 0)[-1]
qplot(x = 1:100, y = t1, geom = 'line');qplot(x = 1:100, y = yield(t1), geom = 'line')
```



Annual precipitation $P$ at time step $t$ is drawn from an approximately Gaussian distribution with user defined mean precipitation $\hat{P}$, coefficient of variation $P_{cv}$, and AR(1) coefficient $\phi$:


where $\epsilon \sim \mathcal{N}(0,\,1)\,$ is a white noise process.

```{r}


set.seed(10)
temporal_patterns <- tibble(time = 1:1000,
       low = accumulate(time, ~weather(.x, phi = .1), .init = 0)[-1],
       high = accumulate(time, ~weather(.x, phi = .6), .init = 0)[-1])
...
set.seed(NULL)
  ...
  temporal_patterns %>%
  gather(phi, SPEI, 2:3) %>%
  mutate(yield = yield(SPEI)) %>%
  mutate(phi = ordered(if_else(phi == 'low', 0.1, 0.6))) %>%
  filter(time <= 500) %>%
  ggplot(aes(time, yield, group = phi, color = phi)) +
  geom_line(size = 1.2) +
    geom_hline(yintercept = 1, color = 'red', linetype = 2) +
  scale_color_viridis_d(direction = -1) +
  facet_wrap(~phi, nrow = 2) +
  labs(color = expression(italic(phi))) +
  theme_classic()
```
...
p1 / (p2 + p3) + plot_annotation(tag_levels = 'A') + plot_layout(heights = c(2, 1))
...
sim <- settlements %>%
  ...
  list(., ., .) %>%
  tibble(net = ., 
         space_base = list( spatial_patterns$grf_01, spatial_patterns$grf_15, spatial_patterns$grf_50)) %>%
  mutate(net = map2(net, space_base, ~mutate(.x, food = .y, anomaly = 0)),
         sim = future_map(net, function(x) reduce(1:nsim, ~run_sim(.x), .init = x))) 
```
...
p1 <- sim[[1,3]] %>%
  nystuen_dacey() %>%
  ggraph(x = x, y = y) +
    geom_sf(data = spatial_patterns, aes(fill = grf_01)) +
  geom_edge_link() +
  scale_size_area() +
  geom_node_point(aes(size = population))+
  coord_sf(datum = NA) +
...
p2 <- sim[[2,3]] %>%
  nystuen_dacey() %>%
  ggraph(x = x, y = y) +
    geom_sf(data = spatial_patterns, aes(fill = grf_15)) +
  geom_edge_link() +
  scale_size_area() +
  geom_node_point(aes(size = population))+
  coord_sf(datum = NA) +
...
p3 <- sim[[3,3]] %>%
  nystuen_dacey() %>%
  ggraph(x = x, y = y) +
    geom_sf(data = spatial_patterns, aes(fill = grf_50)) +
  geom_edge_link() +
  scale_size_area() +
  geom_node_point(aes(size = population))+
  coord_sf(datum = NA) +
