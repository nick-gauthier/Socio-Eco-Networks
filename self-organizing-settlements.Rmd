...
temporal_patterns <- tibble(time = 1:1000,
       `.1` = accumulate(time, ~weather(.x, phi = .1), .init = 0)[-1],
       `.6` = accumulate(time, ~weather(.x, phi = .6), .init = 0)[-1])
...
```{r}
run_sim <- function(net, SPEI = 0, alpha1 = 1.15, alpha2 = 1, beta1 = 5, beta2 = 10){
  new_net <- net %>%
    produce_food(SPEI = SPEI) %>%
...
param_sweep <- expand_grid(beta1 = c(5, 10, 15, 20),
                          ...
                          gamma = c(.85, 1)) %>%
  filter(beta1 <= beta2) %>%
  mutate(sim = future_pmap(list(beta1, beta2, alpha1, alpha2, alpha3, gamma), function(a,b,c,d,e,f) reduce(1:2000, ~run_sim(., beta1 = a, beta2 = b, alpha1 = c, alpha2 = d, alpha3 = e, gamma = f), .init = settlements %N>% mutate(anomaly = 0)), .progress = TRUE))
...
sim4 <- settlements %>%
      prune(beta = 10, tolerance = .001) %N>%
  mutate(EOF = spatial_patterns$oscillation) %>%
  accumulate(temporal_patterns$`.6`, ~run_sim(.x, SPEI = .y), .init = .)
```
