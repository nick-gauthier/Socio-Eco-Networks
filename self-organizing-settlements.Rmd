...
hinterlands <- c(40,-69, -40,-69,-80,0,-40,70,40,69,80,0,40,-69) %>% # changing one 69 in the top corner to a 70 helps for some reason
  ...
  list %>%
  st_polygon %>%
  st_make_grid(cellsize = radius * sqrt(3), n  = 200, square = FALSE) %>% # st_make_grid wants the short diagonal
  st_sf
...
```{r}
interact <- function(net, fluctuation = 1, alpha_t = 1.15, alpha_m = 1.15, beta_t = 0.1, beta_m = 0.05, nu = 0.05, epsilon = .0001, rmax = .02){
  new_net <- net %E>%
    # trade
      mutate(trade_utility = .N()$population[to] ^ alpha_t * exp(-beta_t * distance))  %N>%
    ...
    # migration
    mutate(migrant_utility = (.N()$harvest[to] / .N()$population[to]) ^ alpha_m * exp(-beta_m * distance)) %N>%
    ...
    # population growth
    mutate(population_new = population + .0001 * population * (harvest - population ^ .85) - migrant_production + immigrants, # ^ (1 - 1.15 + 1)
           population_new = population + .02 * population * (1 - population / harvest), #- migrant_production + immigrants , 
           population_new = population + population * pmin(epsilon * (harvest - population ^ .85), rmax), #- migrant_production + immigrants,
...
make_grf <- function(x, y, scale){
  mod <- RandomFields::RMexp(var = 5, scale = scale) + 
    RandomFields::RMnugget(var = 0.2)
    
 RandomFields::RFsimulate(mod, x = pts[,1], y = pts[,2]) %>%
   .$variable1 %>%
   scales::rescale(to = c(1, food_start))
}

RFoptions(seed=100)
env <- paths %>%
  mutate(constant = food_start,
         #random = runif(n(), min = 1, max = food_start),
         grf_001 = make_grf(x, y, 1),
         grf_010 = make_grf(x, y, 10),
         grf_100 = make_grf(x, y, 100),
         gradient = scales::rescale(-1 * node_distance_from(1)),
         gradient2 = scales::rescale(node_distance_from(n)),
...
forcing_function <- expand_grid(time = 1:500,
       omega = c(.01, .05, .3)) %>%
...
p3 <- env %>%
  gather(variable, value, grf_001:grf_100) %>%
  mutate(variable = as.ordered(variable),
         variable = fct_recode(variable,
                               'Autocorrelation scale = 1km' = 'grf_001',
                               'Autocorrelation scale = 10km' = 'grf_010',
                               'Autocorrelation scale = 100km' = 'grf_100')) %>%
  ggplot() +
  geom_sf(aes(fill = value), lwd = .1) +
  facet_wrap(~variable, nrow = 1) +
  theme_void() +
...
param_sweep <- expand_grid(beta_t = c(0, .05, .1, .2, .4),
                           beta_m = c(0, .2, .4, .8, 1)) %>%
  mutate(sim = future_map2(beta_t, beta_m, function(x, y) reduce(1:2000, ~interact(., beta_t = x, beta_m = y), .init = settlements), .progress = TRUE))
